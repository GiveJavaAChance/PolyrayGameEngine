...
struct Viewport2D {
    ...
    Viewport2D(uvec2 s, GLenum colorFormat = GL_RGBA8, GLenum depthFormat = GL_DEPTH_COMPONENT32) : size(s), colorFormat(colorFormat), depthFormat(depthFormat), fbo(size.x, size.y, colorFormat, depthFormat), cam(UINT32_MAX), dirty(true) {
    }
